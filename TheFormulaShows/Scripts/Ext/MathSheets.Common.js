...
MathSheets.Common = MathSheets.Common || (function () {
    ...
    const RESOURCES = (function () {
        var I0001 = '再仔细一点<br/>你可以得到满分',
            I0002 = '看看你的错题<br/>快去订正吧',
            I0003 = '有不会做的题目吗?',
            I0004 = '看来,你以后要多做练习',
            I0005 = '你有些退步了',
            I0006 = '你需要努力了',
    ...
    getControlId = function (id) {
         ...
         * @private
         */
        this._faultInputElement = function () {
         ...
         */
        this._setLocation = function (inputItem) {
            // 設置滾動高度
            $('html,body').animate({
                scrollTop: $(getControlId(inputItem.position)).offset().top
            }, 500, "easeOutQuint", function () {
                // 高亮選中當前輸入框
                if (inputItem.id != null) {
                    $(getControlId(inputItem.id)).focus().select();
                }
            });
        },
         ...
         */
        this._linkSetting = function (style) {
            // 查詢指定的樣式文件并設置為可用
         ...
         * @returns {int} 輸入框的索引號
         */
        this._getNextInputFocusSequence = function (currentSelectedId, keyCode) {
            // 當前focus項目取得
            var f = $("input:focus");
            if (f.length == 0) {
         ...
         */
        this._openWindow = function (url, name, width, height) {
            // 獲取窗口的垂直位置
        ...
        ready = function (btnTheirPapersId, btnReadyId, btnPrintId) {
            // 交卷按鈕顯示
            $(getControlId(btnTheirPapersId)).show();
            // 準備按鈕隱藏
            $(getControlId(btnReadyId)).hide();
            // 打印按鈕隱藏
            $(getControlId(btnPrintId)).hide();
        ...
        pagePrint = function (printDivId) {
            ...
            $(getControlId(printDivId)).printArea(options);
        },
        ...
        printAfterSetting = function (btnTheirPapersId, btnMakeCorrectionsId, btnOverId) {
            ...
            // 交卷按鈕隱藏
            $(getControlId(btnTheirPapersId)).hide();
            // 訂正按鈕隱藏
            $(getControlId(btnMakeCorrectionsId)).hide();
            // 完成按鈕隱藏
            $(getControlId(btnOverId)).hide();
        },
        ...
        lastTimeRestore = function (oldSpanId, rightSpanId, faultSpanId) {
            ...
            if (typeof result !== typeof undefined && result !== false) {
                // 前次用時顯示
                $(getControlId(oldSpanId)).text(result.time);
                // 前次答對數顯示
                $(getControlId(rightSpanId)).text(result.right);
                // 前次答錯數顯示
                $(getControlId(faultSpanId)).text(result.fault);
            }
        },
        ...
        startTime = function (spanSSId, spanMMId, spanHHId) {
            ...
            _second++;
            // 讀秒
            $(getControlId(spanSSId)).text(_checkTime(_second));
            // 讀分
            _minute = parseInt($(getControlId(spanMMId)).text());
            // 60秒后,分鐘加1,秒針歸0
            if (_second == 60) {
                _minute++;
                _second = 0;
                $(getControlId(spanMMId)).text(_checkTime(_minute));
                $(getControlId(spanSSId)).text(_checkTime(_second));
            }
            // 讀時
            _hour = parseInt($(getControlId(spanHHId)).text());
            // 60分后,時針加1,分針歸0
            if (_minute == 60) {
                _hour++;
                _minute = 0;
                $(getControlId(spanHHId)).text(_checkTime(_hour));
                $(getControlId(spanMMId)).text(_checkTime(_minute));
            }
        ...
        makeCorrections = function (fault, btnMakeCorrectionsId, btnOverId) {
            ...
            if (fault != 0) {
                // 存在:訂正按鈕顯示
                $(getControlId(btnMakeCorrectionsId)).show();
                // 錯題項目獲得光標并選中
                _faultInputElement();
            } else {
                // 不存在：訂正按鈕隱藏，完成按鈕顯示
                $(getControlId(btnMakeCorrectionsId)).hide();
                $(getControlId(btnOverId)).show();
            }
        },
        ...
        theirPapers = function (rightSpanId, faultSpanId, btnMakeCorrectionsId, btnOverId, btnTheirPapersId) {
            // 顯示答對題數
            $(getControlId(rightSpanId)).text(__isRight);
            // 顯示答錯題數
            $(getControlId(faultSpanId)).text(__isFault);
            ...
            MathSheets.HelloMrTony.theirPapersComplete(score,
                function (_dialogueArray) {
                    ...
                    if (score >= 8 && score < 10) {
                        ...
                        _dialogueArray.push(String.format("<h5>{0}</h5>", RESOURCES.I0007));
                    }
                });
        },
        ...
        timeStop = function (spanSSId, spanMMId, spanHHId) {
            ...
            // 存儲變量到result鍵（答題結果）
            var timeStr = $(getControlId(spanHHId)).text() + '：' + $(getControlId(spanMMId)).text() + '：' + $(getControlId(spanSSId)).text();
        ...
        forbidKeyDown = function () {
            $(document).bind("keydown", function (e) {
                ...
                if (e.keyCode == 39) {
                    ...
                    _sequence++;
                    $(getControlId(__allInputElementArray[_sequence].id)).focus().select();
