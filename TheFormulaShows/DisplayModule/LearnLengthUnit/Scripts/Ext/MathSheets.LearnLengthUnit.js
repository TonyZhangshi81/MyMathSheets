...
MathSheets.LearnLengthUnit = MathSheets.LearnLengthUnit || (function () {
	...
	printSetting = function () {
		...
		_learnLengthUnitCorrecting = function (pindex, pelement) {
			...
			if (isOK) {
				// 动错题集中移除当前项目
				__allFaultInputElementArray.remove({ position: "mathSheetLearnLengthUnit", id: $(inputAry[0]).attr("id") });
				// 对错图片显示和隐藏
				...
				// 收集所有錯題項目ID
				__allFaultInputElementArray.push({ position: "mathSheetLearnLengthUnit", id: $(inputAry[0]).attr("id") });
				// 对错图片显示和隐藏
